...
def _parse_leon_markets(markets: list) -> Dict:
    ...
    for market in markets:
        ...
        elif 'двойной' in mn and ('шанс' in mn or 'исход' in mn) and 'угловы' not in mn:
            for r in runners:
                if not r.get('open', True) or not r.get('price'): continue
                rn, p = r.get('name', ''), r.get('price', 0)
                if p and p > 1:
        ...
        # === ТОТАЛ ХОЗЯЕВ ===
        elif 'тотал' in mn and 'хозяев' in mn and 'угловы' not in mn and 'карточ' not in mn:
            _ou(runners, 'home')
        
        # === ТОТАЛ ГОСТЕЙ ===
        elif 'тотал' in mn and 'гостей' in mn and 'угловы' not in mn and 'карточ' not in mn:
            _ou(runners, 'away')
        
        # === ТОТАЛ ГОЛОВ (общий) ===
        elif mn == 'тотал' or ('тотал' in mn and 'хозяев' not in mn and 'гостей' not in mn and 'угловы' not in mn and 'карточ' not in mn):
            _ou(runners, 'total')
            open_markets += 1
    
    print(f"Leon parsed: {len(odds)} odds, {open_markets} markets")
    # Log corners and cards specifically
    special = {k: v for k, v in odds.items() if any(x in k for x in ['corners', 'cards', 'dnb', 'first_goal'])}
    if special:
...
def _build_live_markets(odds: dict, home_team: str = '', away_team: str = '') -> list:
    ...
    def _collect(prefix):
        ...
        bets = []
        # Находим все ключи с этим префиксом
        lines_found = set()
        for key in odds:
    ...
    # 6. Кто забьёт первый гол
    if odds.get('first_goal_home'):
        markets.append({
        ...
        if leon_live and leon_live.get("is_live"):
            ...
            return {
                "match": {
                    ...
                    "bet_markets": _build_live_markets(live_odds, leon_live.get("home_team", ""), leon_live.get("away_team", "")),
...
async def get_match_analytics(nocache: bool = False):
    ...
    try:
        ...
        # 5. H2H из FotMob (если есть match_id)
        h2h = []
        if match_id:
...
async def get_match_details(match_id: int):
    """Полная детализация матча из FotMob — events, stats, lineups, ratings"""
    try:
...
async def place_bet_endpoint(bet: BetRequest, user: dict = Depends(get_current_user)):
    ...
    # Определяем коэффициент из Leon API (напрямую, без свопа)
    ...
    if bet.bet_type.startswith('score_'):
        odds = 30.0
    elif leon_odds.get(bet.bet_type):
...
def get_referer_for_url(url: str) -> str:
    ...
    if 'mfvideo' in url or 'myfootball' in url:
        return 'https://myfootball.pw/'
    elif 'liveball' in url:
...
STREAM_FILE = '/app/data/stream.json'

def get_stream_data():
    """Получить данные стрима"""
    try:
...
def save_stream_data(url: str, title: str = ''):
    """Сохранить данные стрима"""
    try:
        ...
        print(f"Save stream error: {e}")
        return False

@app.get("/api/stream")
async def get_stream():
    """Получить текущий стрим для всех пользователей"""
...
async def set_stream(req: StreamRequest, user: dict = Depends(get_current_user)):
    """Установить стрим (только для админов)"""
    if user['user_id'] not in ADMIN_IDS:
        raise HTTPException(status_code=403, detail="Доступ запрещён")
    
    if save_stream_data(req.url, req.title):
        return {"success": True, "url": req.url, "title": req.title}
    else:
